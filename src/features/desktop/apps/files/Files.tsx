...
export const Files: React.FC<FilesProps> = ({ onOpenFile }) => {
    ...
    function handleCreateSubmit(e: React.FormEvent) {
        ...
        if (newItemType === 'folder') {
            ...
            const trimmedExt = extInput.trim().replace(/^\./, '')
            if (!trimmedExt) return 
            createFile(currentFolderId, trimmedName, trimmedExt)
        }
            ...
            <main className="flex flex-1 flex-col">
                ...
                {newItemType && (
                    <form onSubmit={handleCreateSubmit} className="flex items-center gap-2 px-4 py-2 border-b border-taupe-800">
                        ...
                        {newItemType === 'file' && (
                            <>
                                <span className="text-taupe-500 text-xs">.</span>
                                <input
                                    type="text"
                                    value={extInput}
                                    ...
                                    placeholder="txt"
                                    required
                                    className=" border border-taupe-700 rounded px-2 py-1 text-xs outline-none focus:border-white w-14"
                                />
                            </>
                        )}
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            {children.map((node) => (
                                <div
                                    ...
                                    className="group relative flex flex-col items-center gap-1 p-2 rounded-lg cursor-pointer transition-colors"
                                >
                                    <button
                                        onClick={(e) => { e.stopPropagation(); deleteNode(node.id) }}
                                        className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 text-[10px] text-rose-400 hover:text-rose-300 transition"
                                    >
                                        ✕
                                    </button>
                                    <span className="text-3xl">{node.isFolder ? '📁' : '📄'}</span>
                                    <span className="text-xs text-center text-taupe-300 truncate w-full">
                                        {node.name}{!node.isFolder && `.${node.extension}`}
                                    </span>
                                </div>
                            ))}
                        </div>
